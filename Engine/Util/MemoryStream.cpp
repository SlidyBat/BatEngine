...
namespace Bat
{
	...
	MemoryStream MemoryStream::FromFile( FileSystem& filesystem, const std::string& filename )
	{
		ASSERT( filesystem.Exists( filename.c_str() ), "Could not access file %s", filename );

		File file( filesystem, filename.c_str(), "rb" );

		std::vector<char> bytes;
		bytes.resize( file.Size() );
		file.Read( bytes.data(), bytes.size() );

		return MemoryStream( std::move( bytes ) );
	}

	MemoryStream MemoryStream::FromFile( FileSystem& filesystem, const std::wstring& filename )
	{
		return FromFile( filesystem, WideToString( filename ) );
	}
