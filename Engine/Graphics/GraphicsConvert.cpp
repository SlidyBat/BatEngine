...
namespace Bat
{
	...
	IRenderTarget* GraphicsConvert::EquirectangularToCubemap( IGPUContext* pContext, ITexture* equirect, size_t width, size_t height )
	{
		static ConstantBuffer<CB_EquirectangularToCubemapMatrix> transforms;
		CB_EquirectangularToCubemapMatrix transforms_data;

		IRenderTarget* cubemap = gpu->CreateCubemapRenderTarget( width, height, equirect->GetFormat(), 1 );
		...
		for( size_t i = 0; i < 6; i++ )
		{
			...
			pContext->SetRenderTarget( cubemap, i );
